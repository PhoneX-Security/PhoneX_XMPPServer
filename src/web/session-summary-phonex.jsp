...
<%  // Get parameters
    ...
    int range = ParamUtils.getIntParameter(request,"range",webManager.getRowsPerPage("session-summary-phonex", DEFAULT_RANGE));
    int refresh = ParamUtils.getIntParameter(request,"refresh",webManager.getRefreshValue("session-summary-phonex", 0));
    boolean close = ParamUtils.getBooleanParameter(request,"close");
    int order = ParamUtils.getIntParameter(request, "order",
            webManager.getPageProperty("session-summary-phonex", "console.order", SessionResultFilter.ASCENDING));
    String jid = ParamUtils.getParameter(request,"jid");

    if (request.getParameter("range") != null) {
        webManager.setRowsPerPage("session-summary-phonex", range);
    }

    if (request.getParameter("refresh") != null) {
        webManager.setRefreshValue("session-summary-phonex", refresh);
    }

    if (request.getParameter("order") != null) {
        webManager.setPageProperty("session-summary-phonex", "console.order", order);
    }
    ...
    if (close) {
        ...
        response.sendRedirect("session-summary-phonex.jsp?close=success");
        return;
    }
...
<html>
    <head>
        ...
        <meta name="pageID" content="session-summary-phonex"/>
...
<tbody>
<form action="session-summary-phonex.jsp" method="get">
    <tr valign="top">
        <td width="99%">
            ...
            <%  if (numPages > 1) { %>
                ...
                %>
                    <a href="session-summary-phonex.jsp?start=<%= (i*range) %>"
...
<thead>
    <tr>
        ...
        <a href="session-summary-phonex.jsp?order=<%=SessionResultFilter.ASCENDING %>">
        ...
        %>
        <table border="0"><tr valign="middle"><th>
        <a href="session-summary-phonex.jsp?order=<%=SessionResultFilter.DESCENDING %>">
        <fmt:message key="session.details.name" /></a>
        </th><th>
        <a href="session-summary-phonex.jsp?order=<%=SessionResultFilter.DESCENDING %>">
...
<tbody>
    ...
    <%  int count = start;
        ...
        for (ClientSession sess : sessions) {
            ...
            count++;
    %>
        <%@ include file="session-row-phonex.jspf" %>
    <%  } %>
